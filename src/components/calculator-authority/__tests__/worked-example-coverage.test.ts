...
const workedExamplePattern =
  /Worked(?:\s+[A-Za-z-]+)*\s+(?:example|calculation)|Example calculation|Calculation example/i;

describe("calculator worked-example coverage", () => {
  ...
  it.each(calculatorPages)(
    "$slug includes a worked numerical example",
    ({ pagePath }) => {
      const source = readFileSync(pagePath, "utf8");

      expect(source).toMatch(workedExamplePattern);
    },
  );
